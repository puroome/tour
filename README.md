...
| | |
|---|---|
| 읍·면·동 | 3,558개, [js/dong/](./js/dong/)에 시·군별 161개 파일, 합계 약 1.6MB |
| 한 번에 받는 양 | 들어간 시·군의 파일 하나만 — 평균 8KB, 가장 큰 서울이 51KB |
| 시·군 백지도 | [js/map-data.json](./js/map-data.json) 약 650KB, 지도 탭에 처음 들어갈 때 |
...
- 이웃한 읍·면·동을 따로 줄이면 사이에 실틈이 생겨서, 각 경계를 약 50m씩 부풀려 겹치게 메웠습니다. 그래서 경계에서 50m 안쪽은 어느 쪽 읍·면·동으로 잡힐지 정해져 있지 않습니다.
