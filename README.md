...
정답을 맞힌 뒤 퀴즈 카드가 `현장 사진 올리기`로 바뀝니다. **사진 업로드까지 끝나야 탐방 완료로 계산**되므로, 여권·지역·테마 통계와 지도 완료 표시에도 사진 업로드 후에만 반영됩니다. 휴대폰에서는 카메라가 우선 열리고, 사진은 1600px 이하 JPEG로 줄인 뒤 공유 Drive 폴더에 업로드됩니다. 파일명은 학생 `학번-YYYYMMDD_HHMMSS.jpg`이며, `Edit=yes`이고 학번이 비어 있는 관리자는 구글 계정 아이디(이메일 `@` 앞부분)를 학번 자리에 써서 `계정아이디-YYYYMMDD_HHMMSS.jpg`로 저장됩니다. 관리자가 여러 명이어도 서로 구분됩니다.
